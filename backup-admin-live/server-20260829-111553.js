...
// ==================== MÜHENDİSLİK EK ALANLARI ====================

const engineeringColumns = db.prepare(
    "PRAGMA table_info(engineering)"
).all().map(x => x.name);

const extraEngineeringColumns = [
    ["kisa", "TEXT DEFAULT ''"],
    ["nedir", "TEXT DEFAULT ''"],
    ["isler", "TEXT DEFAULT ''"],
    ["dersler", "TEXT DEFAULT ''"],
    ["alanlar", "TEXT DEFAULT ''"],
    ["uygun", "TEXT DEFAULT ''"],
    ["kariyer", "TEXT DEFAULT ''"],
    ["etiket", "TEXT DEFAULT ''"]
];

for (const [column, type] of extraEngineeringColumns) {
    if (!engineeringColumns.includes(column)) {
        db.exec(`ALTER TABLE engineering ADD COLUMN ${column} ${type}`);
    }
}
...
app.get("/api/engineering", (req, res) => {
    ...
    try {
        ...
        const result = rows.map(item => ({
            ...
            kisa: item.kisa || "",
            nedir: item.nedir || item.description || "",

            isler: item.isler
                ? String(item.isler).split("\n").filter(Boolean)
                : (item.work
                    ? String(item.work).split("\n").filter(Boolean)
                    : []),

            dersler: item.dersler
                ? String(item.dersler).split("\n").filter(Boolean)
                : (item.education
                    ? String(item.education).split("\n").filter(Boolean)
                    : []),

            alanlar: item.alanlar
                ? String(item.alanlar).split("\n").filter(Boolean)
                : [],

            uygun: item.uygun || "",
            kariyer: item.kariyer || "",

            etiket: item.etiket
                ? String(item.etiket).split("\n").filter(Boolean)
                : [],
        ...
        res.status(500).json({
            success: false,
            message: "Mühendislikler alınamadı."
        });
...
app.post("/api/admin/engineering", adminAuth, (req, res) => {
    ...
    if (!name) {
        ...
        return res.status(400).json({
            success: false,
            message: "Mühendislik adı gerekli."
        });
    ...
    try {
        ...
        res.json({
            success: true,
            id: result.lastInsertRowid,
            message: "Mühendislik eklendi ve yayınlandı."
        });
...
app.put("/api/admin/engineering/:id", adminAuth, (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            message:
                result.changes > 0
                    ? "Mühendislik güncellendi ve yayınlandı."
